class GeoJsonValidations {
    static isValidPolygon(polygon) {
        ...
        if (Object.getPrototypeOf(polygon) !== Object.prototype) {
            throw new Error("A GeoJson Polygon must be a Object with properties \"type\" and \"coordinates\".")
        }
        if (polygon.type !== 'Polygon') {
            throw new Error("A GeoJson Polygon must have \"type\" property with \"Polygon\" value.")
        }
    ...
    static isValidCoordinate(coordinate) {
        ...
        if (!coordinate.length || (coordinate.length !== 2 && coordinate.length !== 3)) {
            throw new Error("A GeoJson Coordinate must be an Array with two values for longitude and latitude respectively")
        }
        if (isNaN(coordinate[0]) || isNaN(coordinate[1])) {
            throw new Error("Longitude and latitude of a GeoJson Coordinate must be of type Number.")
        }
        if (!(coordinate[0] >= -180 && coordinate[0] <= 180)) {
            throw new Error("Longitude of a GeoJson Coordinate must have values between -180 and 180.")
        }
        if (!(coordinate[1] >= -90 && coordinate[1] <= 90)) {
            throw new Error("Latitude of a GeoJson Coordinate must have values between -90 and 90.")
        }
